...
async function loadPdfLib() {
  const [{ PDFDocument, rgb, PDFFont }, { default: fontkit }] = await Promise.all([
    import("pdf-lib"),
    import("@pdf-lib/fontkit"),
  ])
  return { PDFDocument, rgb, PDFFont, fontkit }
}
...
function fitFontSize(
  ...
  while (size > minSize && font.widthOfTextAtSize(name, size) > maxWidthPt) {
    size -= 0.5
  }
